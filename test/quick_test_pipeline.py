...
def print_pretty_pipeline(result, index=None):
    ...
    if index is not None:
        ...
        print("=" * 75)
        # =============================
    # 🆕 TO-DO ENGINE KẾT QUẢ
    # =============================
    todo_q = result.get("todo_question")
    todo_plan = result.get("todo_plan")
    todo_cands = result.get("todo_candidates")

    if todo_cands:
        print("\n📝 To-Do Candidates phát hiện:")
        for c in todo_cands:
            print(f" - {c['action']} (conf={c.get('confidence', 0)})")
    ...
    if todo_plan:
        print("\n📌 To-Do Plan:")
        print(" 🎯 Main task:", todo_plan.get("main_task"))
        print(" 🪜 Subtasks:")
        for st in todo_plan.get("subtasks", []):
            print("     -", st)
        print(" ⏰ Deadline:", todo_plan.get("deadline"))
        print(" 🗓 Timeline:")
        for tl in todo_plan.get("timeline", []):
            print("     •", tl)
    ...
    if topic and topic.lower() != "không xác định":
        print(f"📘 Topic: {topic}")
    # Source
...
def main(mock_mode=False):
    ...
    test_cases = [
        "Tôi tin bản thân sẽ vượt qua kì thi giữa kì một cách dễ dàng, và thuận lợi, đạt điểm số thật cao"
